...
async function getOwnedGameHours(steamId) {
    ...
    try {
        ...
        const cs2 = games.find(g => g.appid === CS2_APP_ID) //busca CS2 especificamente por appid
        if (!cs2) return 0
        return Math.floor(cs2.playtime_forever / 60) //playtime_forever viene en minutos → convertimos a horas
    } catch (error) {
        return 0
    }
}
...
function parseStats(rawStats){
    ...
    const roundsWon      = get('total_wins')           //rondas ganadas
    const roundsPlayed   = get('total_rounds_played')  //rondas jugadas
    const matchesWon     = get('total_matches_won')    //partidas ganadas
    const matchesPlayed  = get('total_matches_played') //partidas jugadas
    const matchesLost    = matchesPlayed - matchesWon  //partidas perdidas aprox
    const mvps           = get('total_mvps')
    const timePlayed     = get('total_time_played')
    const totalDamage    = get('total_damage_done')    //daño total hecho a enemigos
    ...
    // --- helper para construir stats de un mapa ---
    const mapStats = (mapName) => {
        ...
        return {
            ...
            roundsLost:   rLost,
            winrate:      rPlayed > 0 ? +((rWon / rPlayed) * 100).toFixed(1) : 0, //winrate de rondas en ese mapa
        }
    }

    // --- helper para construir stats de un arma ---
    const weaponStats = (name) => {
        ...
        return {
            ...
            shots:          s,
            hits:           h,
            accuracy:       s > 0 ? +((h / s) * 100).toFixed(1) : 0,  //hits / shots * 100
            bulletsPerKill: k > 0 ? +(s / k).toFixed(1) : 0,          //balas gastadas por kill
        }
    }
    ...
    return {
        ...
        totalDamage,  
        adr:             roundsPlayed > 0 ? +(totalDamage / roundsPlayed).toFixed(1) : 0, //average damage per round
        gunGameRoundsWon,
        winsPistolRound,
        weaponsDonated,
        revenges,
        ...
        weapons: {
            ...
            m4a1:    weaponStats('m4a1'),       //m4a1-s en el juego
            m4a4:    weaponStats('m4a4'),
            awp:     weaponStats('awp'),
            deagle:  weaponStats('deagle'),
            usp:     weaponStats('hkp2000'),    //usp-s se llama hkp2000 en la api
            glock:   weaponStats('glock'),
            famas:   weaponStats('famas'),
            galil:   weaponStats('galilar'),    //galil-ar se llama galilar en la api
            ssg08:   weaponStats('ssg08'),
            mac10:   weaponStats('mac10'),
            mp9:     weaponStats('mp9'),
            mp7:     weaponStats('mp7'),
            negev:   weaponStats('negev'),
            knife:   { kills: get('total_kills_knife'), shots: 0, hits: 0, accuracy: 0, bulletsPerKill: 0 }, //el cuchillo no tiene shots/hits
            sawedoff: weaponStats('sawedoff'),  //escopeta recortada TT
        },

        // --- utilidades y granadas ---
        utility: {
            killsHE:        get('total_kills_hegrenade'),     //kills con granada HE
            killsMolotov:   get('total_kills_molotov'),       //kills con molotov/incendiaria
            killsBlinded:   get('total_kills_enemy_blinded'), //kills a enemigos flasheados
            bombsPlanted:   get('total_planted_bombs'),       //bombas plantadas
            bombsDefused:   get('total_defused_bombs'),       //bombas desactivadas
        },
...
async function getFullPlayerData(steamId, displayName, color) {
    const [summary, rawStats, totalHours] = await Promise.all([
        ...
        getOwnedGameHours(steamId) //horas totales desde el perfil de steam
    ])
    ...
    return {
        ...
        totalHours,     //horas totales que muestra steam en el perfil
        stats
    }
}
